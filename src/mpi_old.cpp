...
void update_position(double *x, double *y, double *local_x, double *local_y,
                     double *vx, double *vy, int start, int end) {
    ...
    for(int i = 0; i < end-start; i++){
        ...
        while(true){
            ...
            if(local_x[i] > bound_x){
                local_x[i] -= 2 * (local_x[i] - bound_x);
                vx[i] = -vx[i];
            }
            if(local_x[i] > 0){
                break;
        ...
        while(true){
            ...
            if(local_y[i] > bound_y){
                local_y[i] -= 2 * (local_y[i] - bound_y);
                vy[i] = -vy[i];
            }
            if(local_y[i] > 0){
                break;
...
void update_velocity(double *m, double *x, double *y, double *vx, double *vy,
                    double *total_vx, double *total_vy,  int start, int end) {
        ...
        for(int j = 0; j < n_body; j++){
            if(i + start != j){
                ...
                if(pow(distance,2) <= 4 * radius2){
                    ...
                    vx[i] = vx[i] + ((F * dt * (x[j] - x[i + start])) / (m[i + start] * distance + err));
                    vy[i] = vy[i] + ((F * dt * (y[j] - y[i + start])) / (m[i + start] * distance + err));
                }
            }
        }
    }
...
void slave(){
    ...
    for (int i = 0; i < n_iteration; i++){
        ...
        if(my_rank == world_size-1){
            update_velocity(total_m, total_x, total_y, local_vx, local_vy, total_vx, total_vy, my_rank * local_size, (my_rank+1) * local_size + extra_length);
            MPI_Barrier(MPI_COMM_WORLD);
            update_position(total_x, total_y, local_x, local_y, local_vx, local_vy, my_rank * local_size, (my_rank+1) * local_size + extra_length);
        }
        else{
            update_velocity(total_m, total_x, total_y, local_vx, local_vy, total_vx, total_vy, my_rank * local_size, (my_rank+1) * local_size);
            MPI_Barrier(MPI_COMM_WORLD);
            update_position(total_x, total_y, local_x, local_y, local_vx, local_vy, my_rank * local_size, (my_rank+1) * local_size);
        }
        MPI_Barrier(MPI_COMM_WORLD);
        // Info gathering
...
void master() {
    ...
    for (int i = 0; i < n_iteration; i++){
        ...
        // TODO: MPI routine
        // Spread the data into different processes.
