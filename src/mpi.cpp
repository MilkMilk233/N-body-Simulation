...
void update_position(double *x, double *y, double *local_x, double *local_y,
                     double *vx, double *vy, double *ax, double *ay, int start, int end) {
...
void update_velocity(double *m, double *x, double *y, double *vx, double *vy,
                    double *total_vx, double *total_vy, double *ax, double *ay, int start, int end) {
...
void slave(){
    ...
    for (int i = 0; i < n_iteration; i++){
        ...
        if(my_rank == world_size-1){
            update_velocity(total_m, total_x, total_y, local_vx, local_vy, total_vx, total_vy, local_ax, local_ay, my_rank * local_size, (my_rank+1) * local_size + extra_length);
            MPI_Barrier(MPI_COMM_WORLD);
            update_position(total_x, total_y, local_x, local_y, local_vx, local_vy, local_ax, local_ay, my_rank * local_size, (my_rank+1) * local_size + extra_length);
        }
        else{
            update_velocity(total_m, total_x, total_y, local_vx, local_vy, total_vx, total_vy, local_ax, local_ay, my_rank * local_size, (my_rank+1) * local_size);
            MPI_Barrier(MPI_COMM_WORLD);
            update_position(total_x, total_y, local_x, local_y, local_vx, local_vy, local_ax, local_ay, my_rank * local_size, (my_rank+1) * local_size);
        }
        MPI_Barrier(MPI_COMM_WORLD);
        // Info gathering
